...
@section('title', 'Monitor IP Ping Sayfası')
    ...
    <script>
        ...
        function updateTable(data) {
            ...
            data.forEach(ip => {
                ...
                monitorTable.row.add([ip.ip, ip.name ?? '', statusBadge, latencyText, deleteBtn]);
            });
        ...
        function showUpdateAnimation() {
            ...
            anim.fadeTo(400, 0.95).delay(2000).fadeOut(600, function() { $(this).remove(); });
        }
        ...
        function loadFailedIPs() {
            ...
            $.getJSON('/monitor/failed-ips', function(data) {
                if (!data.length) failedIpList.append('<li class="list-group-item text-muted">Başarısız IP yok</li>');
                else data.forEach(ip => {
                    failedIpList.append(`<li class="list-group-item list-group-item-danger"><strong>${ip.ip}</strong><br><small class="text-muted">${ip.updated_at} itibariyle başarısız</small></li>`);
                });
            });
        }
            ...
            $.getJSON('/ping/status', function(status) {
                if (!status.running) {
                    $.ajax({
                        ...
                        success: function() { waitForPingResult(0); },
                        error: function(xhr) { console.error('Ping kuyruk işlemi başlatılamadı:', xhr.responseJSON?.message || 'Bilinmeyen hata'); }
                    });
                } else waitForPingResult(0);
            });
        }
        ...
        function queuePing() {
            $.ajax({
                ...
                method: 'POST',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function(response) { alert(response.message); waitForPingResult(); },
                error: function(xhr) { alert('Ping kuyruğa alınamadı: ' + (xhr.responseJSON?.message || 'Bilinmeyen hata')); }
            });
        }

        function waitForPingResult(attempt = 0, maxAttempts = 40) {
            if (attempt >= maxAttempts) { loadIPs(); showUpdateAnimation(); return; }

            setTimeout(() => {
                $.getJSON('/ping/status', function(status) {
                    if (!status.running) { loadIPs(); showUpdateAnimation(); }
                    else waitForPingResult(attempt + 1, maxAttempts);
                });
            }, 3000);
        }
        ...
        $(document).ready(function() {
            ...
            $('#addIpBtn').on('click', function() {
                ...
                if (!ip) return alert('IP adresi boş olamaz.');
                ...
                $.ajax({
                    ...
                    data: { ip, name },
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function() { $('#newIpInput,#newNameInput').val(''); loadIPs(); },
                    error: function(xhr) { alert(xhr.responseJSON?.message || 'IP eklenemedi.'); }
                });
            });

            $('#newIpInput, #newNameInput').on('keypress', function(e) { if (e.which === 13) $('#addIpBtn').click(); });
            ...
            $('#monitorTable tbody').on('click', '.deleteBtn', function() {
                ...
                $.ajax({
                    ...
                    method: 'DELETE',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function() { loadIPs(); },
                    error: function(xhr) { alert('Silinemedi: ' + (xhr.responseJSON?.message || 'Hata oluştu.')); }
                });
            });

            $('#startMonitorBtn').on('click', startMonitoring);
            $('#queuePingBtn').on('click', queuePing);
            $('#goHomeBtn').on('click', function() { window.location.href = '{{ url('/') }}'; });
            $('#toggleFailedIpsBtn').on('click', function() { $('#failedIpsContainer').toggle(); });
        });
    </script>
@endpush
